...
alpha_values = [2.0,3.0,5.0]  # contrast
beta_values = [-50,0,50,80,160]  # brightness

stop_early = False  # flag to exit all loops
...
for alpha in alpha_values:
    for beta in beta_values:
        ...
        totalLabel_results = totalLabel_model(source=bc_image, conf=0.50, save=True, show=True)
        ...
        for idx, totalLabel_result in enumerate(totalLabel_results):
            ...
            if stop_early:
                    break  # exit totalLabel_box loop
            if stop_early:
                break  # exit totalLabel_result loop
        if stop_early:
            break  # exit beta loop
    if stop_early:
