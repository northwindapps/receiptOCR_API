...
def process_image():
    ...
    all_boxes = [
        ...
        (idx, jdx, box, "all_text")
        for idx, crop_result in enumerate(crop_all_results)
        for jdx, box in enumerate(crop_result.boxes)
    ]

    for idx, jdx, total_box, kind in all_boxes:
        ...
        thresh = cv2.adaptiveThreshold(
            g, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 25, 4
        )
        ...
        decoded_str = decode_with_crnn(thresh, parse_model)

        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        crop_filename = f"{kind}_{decoded_str}_{idx}_{jdx}_{timestamp}.png"
        cv2.imwrite(os.path.join(SAVE_DIR, crop_filename), thresh)

        results.append({"kind": kind, "text": decoded_str, "crop_file": crop_filename})
