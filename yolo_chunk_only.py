...
chunk_results = chunk_model(source=image, conf=0.5, save=True, show=True)
...
for idx,chunk_result in enumerate(chunk_results):
    for jdx, chunk_box in enumerate(chunk_result.boxes):
        ...
        for cont_value in cont_area_values:
            alpha_stop = False
            layout_crop = image[int(y_min):int(y_max), int(x_min):int(x_max)]    
            for av in alpha_values:
                ...
                for scale in scales:
                    ...
                    for bv in beta_values:
                        if stop_early:
                            break 
                        processed_img = cv2.resize(layout_crop, None, fx=scale, fy=scale, interpolation=cv2.INTER_CUBIC)
                        ...
                        # # Unsharp masking
                        # blurred = cv2.GaussianBlur(processed_img, blur_size, sigma)
                        # processed_img = cv2.addWeighted(processed_img, 1 + strength, blurred, -strength, 0)
                        ...
                        if best_text == '' or not any(char.isdigit() for char in best_text):
                            stop_early = True
